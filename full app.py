...
def setup_initial_frame(root):
    ...
    start_button = tk.Button(bottom_frame, text="Get Started", font=("Britannic", 20, "bold"),
                             fg="seagreen", bg="darkseagreen", width=20, height=2, command=lambda: show_reward_option(root))
...
def show_reward_option(root):
    ...
    enter_reward_button = tk.Button(initial_frame, text="Enter Reward", command=lambda: show_reward_frame(root),
                                    bg="darkseagreen", font=("Britannic", 12, "bold"), fg="seagreen", bd=3)
    enter_reward_button.pack(pady=10)

    skip_reward_button = tk.Button(initial_frame, text="Skip Reward", command=lambda: update_progress(root, None),
                                   bg="darkseagreen", font=("Britannic", 12, "bold"), fg="seagreen", bd=3)
